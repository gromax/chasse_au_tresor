  ...
  public static function getList($options = array())
  {
    ...
    try {
      if (isset($options['redacteur']))
      {
        ...
        return DB::query("SELECT i.id, i.idEvenement, i.hash, i.ext FROM ((".PREFIX_BDD."images i JOIN ".PREFIX_BDD."evenements e ON e.id = i.idEvenement) LEFT JOIN ".PREFIX_BDD."partages s ON s.idEvenement = e.id  AND s.idRedacteur=%i) WHERE e.idProprietaire=%i OR NOT s.id IS NULL", $idRedacteur, $idRedacteur);
      }
      elseif (isset($options['evenement']))
      {
